...
# # get P(k) from cll abcco emu with baryonic boost 
# baryons = ccl.BaccoemuBaryons()
# cosmo_baryons = ccl.Cosmology(Omega_c=Omega_c_fid, Omega_b=Omega_b_fid, h=h_fid, n_s=ns_fid, A_s=log10As_fid,
#                       m_nu=Mnu_fid, transfer_function='boltzmann_camb',
#                       matter_power_spectrum=baryons)

# tracers
lens = []
cluster = []
bias_ia = np.zeros(len(MGLtest.Survey.zz_integr))  #0.42*np.ones_like(MGLtest.Survey.zz_integr) #A_IA=1 in the NLA model
# bias_ia = 0.42 * ((1 + MGLtest.Survey.zz_integr) / (1 + 0.62))**(2.21)  # NLA-z
...
# ----------------------------------- #
# Add here ccl/mgl plot with errorbars
# ----------------------------------- #
