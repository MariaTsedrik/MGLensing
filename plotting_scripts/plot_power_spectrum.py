...
def plot_pgg(z_int_pick, bini, binj, pgg_list, labels, show=True, name=""):
    ...
    for i in range(3):
        for j, pgg in enumerate(pgg_list):
            ax[0][i].loglog(k_ell[:, z_int_pick[i]], pgg[:, z_int_pick[i], bini-1, binj-1], label=labels[j] if i == 0 else '', linestyle='-')
            ax[1][i].semilogx(k_ell[:, z_int_pick[i]], pgg[:, z_int_pick[i], bini-1, binj-1]/pgg_list[0][:, z_int_pick[i], bini-1, binj-1], linestyle='-')
        if i==2:
            ax[0][i].legend(title=f'$z={zz[z_int_pick[i]]:.2f}$ in bin {bini}-{binj}\n $b_{{s^2}} = {bs2L_rsd_arr[bini-1]}, b_{{\\nabla^2}} = {blaplL_rsd_arr[bini-1]}$', loc='upper right', frameon=True)
        else:    
            ax[0][i].legend(title=f'$z={zz[z_int_pick[i]]:.2f}$ in bin {bini}-{binj}', loc='lower left', frameon=True)
        ax[1][i].set_xlabel("$k$ [$h$/Mpc]")
        secax = ax[0][i].secondary_xaxis('top', functions=(f_arr[i], f_inv_arr[i]))
        secax.set_xlabel('$\ell$')
        ax[0, i].axvspan(xmin=0.7, xmax=k_ell[-1, z_int_pick[i]], color='grey', alpha=0.1)
        ax[1, i].axvspan(xmin=0.7, xmax=k_ell[-1, z_int_pick[i]], color='grey', alpha=0.1)
...
# pick 3 redshifts
z_int_pick = [20, 80, 100] #goes from 0 to 255
...
models = {
    ...
    'heft_full_nl_bar': {'nl_model': NL_MODEL_BACCO, 'bias_model': BIAS_HEFT_PNL_BAR, 'ia_model': 0, 'baryon_model': BARYONS_BACCO, 'photoz_err_model': 0., 'heft_extrap_k_const': False},
...
for bin_i in range(nbin):
    params[f'b2L_{bin_i+1}']=b2L_post_arr[bin_i]
_, _, _, heft_nl_b1b2_post = MGL.get_power_spectra(params, models['heft_full_nl'])
_, _, _, heft_nl_b1b2_post_bar = MGL.get_power_spectra(params, models['heft_full_nl_bar'])
params['log10Mc_bc']=15.
_, _, _, heft_nl_b1b2_post_bar_15 = MGL.get_power_spectra(params, models['heft_full_nl_bar'])
params['log10Mc_bc']=10.
_, _, _, heft_nl_b1b2_post_bar_10 = MGL.get_power_spectra(params, models['heft_full_nl_bar'])
for bin_i in range(nbin):
    params[f'b2L_{bin_i+1}']=b2L_rsd_arr[bin_i]
#print(params)    
_, _, _, heft_b1b2 = MGL.get_power_spectra(params, models['heft_full'])
#print(heft_b1b2.shape)
_, _, _, heft_nl_b1b2 = MGL.get_power_spectra(params, models['heft_full_nl'])
for bin_i in range(nbin):
    params[f'bs2L_{bin_i+1}']=bs2L_rsd_arr[bin_i]
    params[f'blaplL_{bin_i+1}']=blaplL_rsd_arr[bin_i]
k_ell, _, _, heft_full = MGL.get_power_spectra(params, models['heft_full'])
...
plot_pgg(z_int_pick, bin_i_plot, bin_j_plot, [heft_full, heft_b1b2, 
                                              heft_full_nl, heft_nl_b1b2, 
                                              heft_nl_b1b2_post, #heft_nl_b1b2_post_bar, 
                                              #heft_nl_b1b2_post_bar_15, heft_nl_b1b2_post_bar_10
                                              ], 
         ['heft: $b_1, b_2, b_{s^2}, b_{\\nabla^2}$', 'heft: $b_{s^2}=b_{\\nabla^2}=0$', 
          'DES: $b_1, b_2, b_{s^2}, b_{\\nabla^2}$', 'DES: $b_{s^2}=b_{\\nabla^2}=0$',
          'DES: $b_1, \hat{b}^{\\rm maxlike}_2$, $b_{s^2}=b_{\\nabla^2}=0$',
          #'DES: $b_1, \hat{b}^{\\rm maxlike}_2$, log10Mc=14.2',
          #'DES: $b_1, \hat{b}^{\\rm maxlike}_2$, log10Mc=15',
          #'DES: $b_1, \hat{b}^{\\rm maxlike}_2$, log10Mc=10'
          ], show=True, name='heft_full_nl_bar_comparison')
