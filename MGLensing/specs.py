...
def get_luminosity_func(file='scaledmeanlum_E2Sa.dat'):
    """
    ...
    -----------
    file : str, optional
        The path to the file containing the luminosity function data. Default is 'scaledmeanlum_E2Sa.dat'.
...
class EuclidSetUp:
    ...
    def __init__(self, survey_info, scale_cuts_info):
        ...
        lmax = 5000
        self.lmax = lmax
        self.ell = np.logspace(np.log10(lmin), np.log10(lmax), num=self.lbin, endpoint=True) 
        validate_and_setup_lmax(self, scale_cuts_info, lmin, lmax, zz_mod_wl, zz_mod_gg)
        self.lum_func = get_luminosity_func(file='scaledmeanlum_E2Sa.dat')
...
class LSSTSetUp:
    ...
    def __init__(self, survey_info, scale_cuts_info):
        ...
        if survey_info == 'LSST_Y1':
            self.nbin = 5 
            nz_sacc_file = './forecast_fid.sacc'
            ...
            nz_sacc_file = './forecast_fid.sacc' # change later
    ...
    def get_norm_galaxy_distrib(self, file='forecast_fid.sacc'):
        """
        Reads the source and lens galaxy normalised distributions from a sacc file.
